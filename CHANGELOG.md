...
#### タイル右上にハートボタン
- 設定画面の保存履歴グリッド／保存ウィンドウ内の保存履歴グリッドの両方で、各タイル右上に ❤️/🤍 ハートボタンを追加（クリックでお気に入りトグル）
- データ構造：`saveHistory[i].favorite: boolean`（未設定時は false 扱い）
- チェックボックスは設定画面・保存ウィンドウともに**左上**に統一（保存ウィンドウ側は v1.33.0 で右上にあったものを左上へ移動）

#### 一括お気に入り追加／解除
- 設定画面・保存ウィンドウ両方の保存履歴ツールバーに「❤️ お気に入り追加」「🤍 お気に入り解除」ボタンを追加
- 選択件数 > 0 で活性化、明示的に追加と解除を 2 ボタンに分離（混在選択時の意図的操作）

#### お気に入りのみ表示する独立フィルタ
- 「🤍 お気に入りのみ」トグルボタンを設定画面・保存ウィンドウ両方の絞り込み行に追加
- 既存のタグ／権利者／取り込み元／表示形式フィルタとは AND 結合で絞り込み

### Added — 保存ウィンドウに全選択／選択解除ボタン（GROUP-37）

- 保存ウィンドウの保存履歴ツールバーに「✅ 全選択」「☐ 選択解除」ボタンを追加（v1.33.2 パターン：絞り込み中は絞り込み結果を対象）
- 全選択：絞り込み結果に「未選択」が 1 件以上ある時のみ活性、すべて選択済または絞り込み結果 0 件で非活性
- 選択解除：選択件数 > 0 で活性

### Improved — 一括操作の処理中表示（GROUP-38）

#### 背景
v1.36.0 までは、グループ化／解除など一括操作中：
1. 視覚的な進行表示がない
2. 選択チェックが操作完了まで残ったまま見える
3. ボタンが活性状態のままに見え二重押下のリスク

ユーザー報告（v1.36.0 リリース時）：
> 「グループ化はかなり軽くなった気がします。ただ、時間は多少かかるので、何らかの表示はしたほうがよいかもしれませんね。グループ化・解除、ともに完了するまで選択解除されない(少なくとも表示上は)ので各種ボタンが活性状態のままに見えるのも気になります。」

#### 実装
**設定画面**：`#busy-modal-overlay` を追加。一括操作の冒頭で `showBusyModal(message, sub)` を呼び、`finally` で `hideBusyModal()`。重い／遅い処理中は半透明オーバーレイ＋スピナー＋件数を表示し、ユーザー入力をブロック。

適用先：
- `hist-delete-selected`（選択削除）
- `hist-group-selected` / `hist-ungroup-selected`（グループ化・解除）
- `hist-add-tag-selected`（一括タグ追加 OK 時）
- `hist-add-author-selected`（一括権利者追加 OK 時）
- `hist-replace-selected` / `hist-remove-selected`（一括置換・除去 OK 時）
- `hist-sync-global-tags`（タグ・保存先反映）
- `hist-fav-add-selected` / `hist-fav-remove-selected`（一括お気に入り追加・解除）

**保存ウィンドウ**：shadow DOM 構造のため独自オーバーレイは追加せず、お気に入り一括ボタンに対し「処理中…」テキスト切替＋ disabled で類似効果を提供。

### Files Changed
- `manifest.json`：version 1.36.0 → 1.37.0
- `src/settings/settings.js`：お気に入り機能（toggle / bulk / filter）／ busy modal helper ＋ 重い一括操作への適用
- `src/settings/settings.html`：ハートボタン CSS／お気に入りトグル ＋ 一括ボタン UI／busy modal オーバーレイ
- `src/modal/modal.js`：チェックボックス左上化／ハートボタン CSS＋ HTML／お気に入り toggle / bulk / filter／全選択・選択解除ボタン
