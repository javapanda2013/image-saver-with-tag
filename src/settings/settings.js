...
// 保存履歴 DOM を debounced refresh（_historyData 再 read + renderHistoryGrid 全件再描画、
...
async function exportData() {
  ...
  // v1.41.6:tagRecords をエクスポート対象から除外（write-only 監査記録、機能経路ゼロ、saveHistory に冗長）
...
async function importData(e) {
  ...
  let current;
  try {
    // v1.41.6:tagRecords を取得対象から除外（インポート時のマージも廃止）
  ...
  // v1.41.6:tagRecords は廃止（write-only 監査記録、saveHistory に冗長）。
...
// v1.46.14 Phase C-3：HISTORY_ENTRY_* listener から呼ばれる setter。
// 別 context（modal 等）が saveHistory を変更したときに、受信側 settings.js の
// _historyData を fresh に置換し、renderHistoryGrid で全件再描画。
// 真の entry レベル DOM 部分更新は Phase C-3-opt で対応予定。
...
function setupHistoryTab() {
  ...
  // v1.38.0：お気に入り画像プレビュー＋完了状態対応（）
  // v1.38.1：完了時の auto-close を削除（閉じる操作は明示操作専用）
  // // ライフサイクル：
  // showBusyModal(msg, sub)  → 表示開始（spinner＋お気に入りプレビュー）
  // completeBusyModal(doneMsg) → 「✅ 完了」へ遷移、閉じるボタン表示。**閉じるのは明示操作専用**
...
async function renderHistoryTab() {
  ...
  const stored = { saveHistory: _hist, ...(await browser.storage.local.get([
    ...
    "histFilenameFilter",
    // v1.46.19 GROUP-75-grid-resizable：保存履歴 grid 幅の永続値
...
// v1.46.19 GROUP-75-grid-resizable → v1.46.20 GROUP-78 改修：
...
function _extB1SetupEvents() {
  ...
  // v1.25.3: ループ構造化（最終ページから「次」で先頭、先頭から「前」で最終ページへ）
...
async function _extB1LoadCurrent() {
  ...
  // pending が残っていれば完了処理にはしない（フィルタ変更で再開できるよう）
...
function _extB1HandleArrowKey(event) {
  ...
  if (thumbsModal && thumbsModal.style.display === "flex") {
    ...
    // （v1.25.3 で誤って全面無効化していた。本来はスクロールイベント側のみ抑制）
